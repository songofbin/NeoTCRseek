...
# Argument parsing
if len(sys.argv) != 6:
    print(f"Usage: python {sys.argv[0]} <expanded_TCRs.anno.txt> <cluster.txt> <eps_name> <prop_cutoff> <rate_cutoff>")
    sys.exit(1)

infile1 = sys.argv[1]
infile2 = sys.argv[2]
eps_name = sys.argv[3]
prop_cutoff = float(sys.argv[4])
rate_cutoff = float(sys.argv[5])

outfile = Path(infile2).with_suffix("").as_posix() + "_" + eps_name + ".tsv"

def select_detect_and_cluster_TCRs(infile1, infile2, outfile, eps_name, prop_cutoff, rate_cutoff):
    # Step 1: Load data
    df1 = pd.read_csv(infile1, sep='\t')
    df2 = pd.read_csv(infile2, sep='\t')
    df2 = df2.rename(columns={eps_name: 'cluster'})[['cdr3aa', 'cluster']]
    ...
    # Step 3: Cluster purity
    df_cluster_purity = df_keep.groupby('cluster')['Detected'].mean().reset_index(name='cluster_purity')
    df_keep = pd.merge(df_keep, df_cluster_purity, on='cluster', how='left')

    # Step 4: V gene filtering
    df_detect = df_keep[df_keep.Detected == True]
    ...
    df_keep2 = df_keep[df_keep['cluster'].isin(filtered_clusters)]
    ...
    print(eps_name, true_num, false_num, sep="\t", end="\t")

# Run the function
select_detect_and_cluster_TCRs(infile1, infile2, outfile, eps_name, prop_cutoff, rate_cutoff)
print()
